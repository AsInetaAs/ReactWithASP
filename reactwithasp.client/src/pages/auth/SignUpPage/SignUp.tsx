    ...
    return (
        <form onSubmit={handleSubmit(storeUser)} className="flex flex-col gap-3 max-w-xs">
            ...
            <div>
                ...
                <input id="userName" className={formStyle.input} {...register("userName", { required: "UserName is required", maxLength: { value: 20, message: 'UserName cannot exceed 20 characters' } })} />
            ...
            <div>
                ...
                <input type="password" id="password" className={formStyle.input} {...register("password", { required: 'Password is required', minLength: { value: 5, message: 'Password must be at least 5 characters long.' }, maxLength: { value: 9, message: 'Password cannot exceed 9 characters' } })} />
                <ErrorBlock errors={errors} name="password" />
            </div>
            <div>
                <label htmlFor="confirm_password" className={formStyle.label}>Confirm password</label>
                <input type="password" id="confirm_password" className={formStyle.input} {...register("confirm_password", { required: 'Confirm password is required', validate: (val: string) => { if (watch('password') != val) { return "Your passwords do no match" } } })} />
                <ErrorBlock errors={errors} name="confirm_password" />
            </div>
            <button type="submit" className={formStyle.button}>Create</button>
        </form>
    )
